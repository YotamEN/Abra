from abra.errors import UnsupportedMessageQueueError
...
class RabbitMQHandler:
    ...
    def __init__(self, q_path):
        ...
        if self.q_f_url.scheme not in SUPPORTED_MQS:
            raise UnsupportedMessageQueueError(f'ERROR: only supported message queues are: {SUPPORTED_MQS!r}')
    ...
    def consume_queue(self, queue, callback):
        self.declare_parser_queue(queue=queue)
    ...
    def publish_to_parsers(self, msg):
        ...
        # print(f"Published to {RB_EXCHANGE_NAME} exchange!")

    # *********************************************
    # ************* methods on saver  *************
    # *********************************************
    def declare_parsed_data_exchange(self):
        self.channel.exchange_declare(exchange=PARSED_DATA_EXCHANGE_NAME, exchange_type="topic")
    ...
    def consume_parsed_data_exchange_by_topic(self, callback, topics):
        ...
        # print(f"Registering to topics: {topics!r}")
